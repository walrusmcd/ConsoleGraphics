...
void f_graph::showTextInBox(const char *_shownInfo,bool _newBeginKey,bool _controlKey, int _x1, int _y1, int _x2, int _y2)
{
...
void f_graph::showLeftSide(const char * _shownInfo,bool _newBeginKey)
{
     CONSOLE_SCREEN_BUFFER_INFO screenInfo;
     GetConsoleScreenBufferInfo(consoleHandler, &screenInfo);
...
void f_graph::showRightSide(const char *_shownInfo,bool _newBeginKey,bool _controlKey)
{
...
void f_graph::headerLine(const char * _headString)
{
    setCursor(CURSOR_OFF);
    int xBegin = whereX();
    int yBegin = whereY();
	...
	if(strLength<=67)
	{
     ...
     {
         const char *lengthWarning = "Your string's length can not be larger than 67 character.";
...
void f_graph::footerLine(const char *_footString)
{
    int strLength;
...
void f_graph::rightBottomText(const char * _headerText,const WORD _textColor)
{
    setCursor(CURSOR_OFF);
    int xBegin = whereX();
    int yBegin = whereY();
    setColor(_textColor);
	const char *ptrLetter;
	int ctr=0;
	int xAxes = 76;
...
void f_graph::blink(const char * _blinkText,const WORD _textColor)
{
     CONSOLE_SCREEN_BUFFER_INFO screenInfo;
     GetConsoleScreenBufferInfo(consoleHandler, &screenInfo);
     setCursor(CURSOR_OFF);
...
void f_graph::warning(const char * _warningText)
{
     blink(_warningText,FOREGROUND_RED | FOREGROUND_INTENSITY);
     };

// to blink red at the bottom
void f_graph::message(const char * _messageText)
{
