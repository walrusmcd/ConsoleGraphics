...
public:
   ...
   void showTextInBox(const char *_shownInfo,bool _newBeginKey,bool _controlKey ,int _x1,int _y1,int _x2,int _y2);
   
   // to show given items, as menu items on the left side
   void showLeftSide(const char * _shownInfo,bool _newBeginKey = 0);
   
   // to show given information on the right box - (it means screen)
   void showRightSide(const char *_shownInfo,bool _newBeginKey = 0,bool _controlKey = 1);
   ...
   void headerLine(const char * _headerText);
   
   // to set the text at the bottom
   void footerLine(const char *_footerText);
   
   // to set the text that out of the boxes, <bottom right corner>
   void rightBottomText(const char * _headerText,const WORD _textColor = FOREGROUND_GREEN | FOREGROUND_INTENSITY);
   
   // to make a blinked text at the bottom, out of boxes
   void blink(const char * _blinkText,const WORD _textColor =  FOREGROUND_RED |  FOREGROUND_INTENSITY);
   
   // to blink red at the bottom
   void warning(const char * _warningText);
   
   // to blink blue at the bottom
   void message(const char * _messageText);
